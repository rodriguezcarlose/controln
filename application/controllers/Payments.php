...
class Payments extends CI_Controller
{
    ...
    public function individualload()
    {
        ...
        // inicio validaciones
        $this->form_validation->set_rules('beneficiario', 'beneficiario', 'required|alpha_spaces', array(
            'required' => 'El Campo beneficiario es requerido.',
            'alpha_spaces' => 'El Campo beneficiario debe ser alfabetico.'
        ));
        $this->form_validation->set_rules('tipoDocumentoIdentidad', 'tipoDocumentoIdentidad', 'required', array(
            'required' => 'El Campo Letra RIF/CI es requerido.'
        ));
        $this->form_validation->set_rules('id_cargo', 'id_cargo', 'required', array(
            'required' => 'El Campo Cargo es requerido.'
        ));
        $this->form_validation->set_rules('rifci', 'rifci', 'required|numeric|max_length[9]', array(
            'required' => 'El Campo Nro. RIF/CI es requerido',
            'numeric' => 'El Campo Nro. RIF/CI debe ser n&uacutemerico.',
            'max_length' => 'El Campo Nro. RIF/CI no debe ser mayor a 9 disgitos.'
        ));
        $this->form_validation->set_rules('tipocuenta', 'tipocuenta', 'required', array(
            'required' => 'El Campo Tipo de Cuenta es requerido.'
        ));
        $this->form_validation->set_rules('banco', 'banco', 'required', array(
            'required' => 'El Campo Banco es requerido.'
        ));
        $this->form_validation->set_rules('cuenta', 'cuenta', 'required|numeric|exact_length[20]', array(
            'required' => 'El Campo Nro. Cuenta es requerido.',
            'numeric' => 'El Campo Nro. Cuenta debe ser n&uacute;merico.',
            'exact_length' => 'El Campo Nro. Cuenta debe ser de 20 digitos.'
        ));
        $this->form_validation->set_rules('monto', 'monto', 'required|numeric', array(
            'required' => 'El Monto es requerido.',
            'numeric' => 'El Monto deber ser n&uacutemerico.'
        ));
    ...
    public function guardarIndividualload()
    {
        ...
        $this->form_validation->set_rules('descripcion', 'descripcion', 'required|alpha_numeric_spaces', array(
            'required' => 'El Campo Descripcion es requerido.',
            'alpha_spaces' => 'El Campo Descripcion debe ser alfanumerico.'
        ));
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_SESSION['addrecords'])) {
                ...
                $resultado = $this->payments_model->insertPaymentIndividual($this->input->post("descripcion"), $this->input->post("id_proyecto"), $this->input->post("id_gerencia"), $_SESSION['id'], $_SESSION['addrecords']);
                if ($resultado) {
                    ...
                    $to_subject = $this->payments_model->getEmailsubject($resultado);
                    {
                        ...
                        $descripciong = $this->input->post("descripcion");
                        /*
                         * $subject = "";
                         * foreach ($to_subject->result() as $descripcionx){
                         * $subject = $descripcionx->descripcion;
                         * }
                         */
                        $gerencianomina = "";
                        foreach ($to_subject->result() as $nombregerencia) {
    ...
    public function loadgrid()
    {
        ...
        $this->form_validation->set_rules('descripcion', 'descripcion', 'required', array(
            'required' => 'El Campo Descripci&oacute;n N&oacute;mina es requerido'
        ));
        $this->form_validation->set_rules('id_proyecto', 'id_proyecto', 'required', array(
            'required' => 'El Campo Proyecto es requerido'
        ));
    ...
    public function guardar($descripcion, $proyecto, $gerencia)
    {
        ...
        $prueba = $this->payments_model->gettotaltable($tablename);
        $credito = (float) $prueba[0]->credito;
        
        $pprueba = $this->Salary_model->gettotal($this->input->post("id_gerencia"));
        $total = (float) $pprueba[0]->total;
        
        if (isset($_SESSION['table_temp_nom'])) {
            $detalle = $this->payments_model->getTablepaymentsTem($_SESSION['table_temp_nom']);
            
            if ($credito <= $total) {
                $resultado = $this->payments_model->insertPayment($descripcion, $proyecto, $gerencia, $_SESSION['id'], $detalle->result(), $total);
                if ($resultado) {
                ...
                $data->error = 'El Monto fijado para esta Gerencia supera el limite establecido, por favor Contacte a Administracion.';
            }
        } else {
            $data->error = 'Ha acorrido un error inesperado, tabla origen no encontrada. Por favor intente de nuevo.';
        }
    ...
    public function do_upload_xls()
    {
        ...
        if (! $this->upload->do_upload('userfile')) {
            ...
            for ($row = 2; $row <= $highestRow; $row ++) {
                ...
                // str_replace("\n", '<p>', $str)
                $fila = array(
                    "beneficiario" => str_replace("\n", '', strtoupper($this->form_validation->stripAccents($sheet->getCell("A" . $row)
                        ->getValue()))),
    ...
    private function validateCSV($params)
    {
        ...
        foreach ($params as $validateparams) {
            ...
            ($this->form_validation->numeric($validateparams->documento_identidad) && strlen($validateparams->documento_identidad) <= 9 && $this->form_validation->required($validateparams->documento_identidad)) == true ? $validateparams->vdocumento_identidad = true : $validateparams->vdocumento_identidad = false;
            ...
            ($this->form_validation->numeric($validateparams->numero_cuenta) && strlen($validateparams->numero_cuenta) == 20 && $this->form_validation->required($validateparams->numero_cuenta)) == true ? $validateparams->vnumero_cuenta = true : $validateparams->vnumero_cuenta = false;
            ...
            ($this->form_validation->numeric($credito) && $this->form_validation->required($validateparams->credito) && $credito > 0) == true ? $validateparams->vcredito = true : $validateparams->vcredito = false;
